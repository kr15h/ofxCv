...
namespace ofxCv {
	...
	vector<glm::vec3> FlowPyrLK::getFeatures(){
		ofPolyline poly;
		for(auto i=0; i<prevPts.size(); ++i){
			poly.addVertex(prevPts[i].x, prevPts[i].y);
		}
		return poly.getVertices();
	}
	
	vector<glm::vec3> FlowPyrLK::getCurrent(){
		vector<glm::vec3> ret;
		for(int i = 0; i < (int)nextPts.size(); i++) {
			if(status[i]){
				ret.push_back(glm::vec3(nextPts[i].x,nextPts[i].y,0));
			}
		}
		return ret;
	}
	...
	vector<glm::vec2> FlowPyrLK::getMotion(){
		...
		for(int i = 0; i < (int)prevPts.size(); i++) {
			if(status[i]){
				ret.push_back(
					glm::vec2(nextPts[i].x,nextPts[i].y)-
					glm::vec2(prevPts[i].x,prevPts[i].y));
			}
		}
		return ret;
	}
	...
	void FlowPyrLK::drawFlow(ofRectangle rect) {
		...
		for(int i = 0; i < (int)prevPts.size(); i++) {
			if(status[i]){
				ofDrawLine(
					glm::vec2(prevPts[i].x,prevPts[i].y)*scale+offset, 
					glm::vec2(nextPts[i].x,nextPts[i].y)*scale+offset);
			}
		}
	}
